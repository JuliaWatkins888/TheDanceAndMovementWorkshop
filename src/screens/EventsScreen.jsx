...
useEffect(()=>{console.log(events)}, [events])
  return (
    <Box style={{ display: 'flex', width: isMobile ? '100vw' : 'calc(100vw - 32px)', height: '100vh', zIndex: 2, margin: isMobile ? '80px 0px 0px 0px' : '80px 16px 16px 16px', overflow: 'scroll' }}>
      <Box style={{ display: 'flex', flexWrap: 'wrap', justifyContent: isMobile ? 'center' : 'space-around', alignItems: events.length === 0 && 'center', width: '100%' }}>
        {events.length === 0 && <Typography>We're sorry, currently there are no events scheduled.</Typography>}
        {events.map((event) => (
